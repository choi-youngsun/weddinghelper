...
import GroomGuest from '@/db/models/groomGuest';
...
export default authenticate(async function handler(
  ...
  if (req.method === 'PATCH') {
    ...
    try {
      // 1. GroomGuest 데이터 수정
      const guest = await GroomGuest.findOne({ _id: id });
      ...
      const user = await UserData.findOneAndUpdate(
        { _id: userId, 'groomGuests._id': id }, // 배열의 특정 객체를 찾는 조건
        {
          $set: {
            'groomGuests.$.guestName': guestName,
            'groomGuests.$.affiliation': affiliation,
            'groomGuests.$.giftAmount': giftAmount,
            'groomGuests.$.ticketCount': ticketCount,
            'groomGuests.$.note': note,
          },
        },
      ...
      if (!user) {
        return res.status(404).json({
          success: false,
          message: 'User not found or Guest not in groomGuests',
        });
      }
      res.status(200).json({
        success: true,
    ...
    try {
      const guest = await GroomGuest.findByIdAndDelete({ _id: id });
      ...
      // groomGuests 배열에서 _id가 일치하는 객체 삭제
      const updatedUser = await UserData.findByIdAndUpdate(
        userId,
        { $pull: { groomGuests: { _id: id } } }, // 조건 설정
